...
func NewRanking() *Ranking {
	return &Ranking{scoreMap: map[string]int64{}}
}

// Set 设置 key 对应的分数
// 排名由大到小，得分相同情况下，最新更新的 key 排名更靠前
func (r *Ranking) Set(key string, score int64) {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *Ranking) GetRange(from, to int) ([]string, []int64) {
	r.lock.RLock()
	defer r.lock.RUnlock()
	if to < from {
		to = from
	}
	return getRange(r.tree, from, to, []string{}, []int64{})
}

// 获取排名为 n 的 key,score
// n>0 && n<=ranking.len，否则返回 "",0
func (r *Ranking) GetN(n int) (string, int64) {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
// index 从 1 开始
func (r *Ranking) Walk(handler func(index int, key string, score int64)) {
	r.lock.RLock()
	defer r.lock.RUnlock()
