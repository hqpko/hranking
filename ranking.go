...
type Ranking struct {
	lock     sync.RWMutex
	tree     *tree
	scoreMap map[Key]Value
}

func NewRanking() *Ranking {
	return &Ranking{scoreMap: map[Key]Value{}}
}

// Set 设置 key 对应的分数
// 排名由大到小，得分相同情况下，最新更新的 key 排名更靠前
func (r *Ranking) Set(key Key, score Value) {
	r.lock.Lock()
	defer r.lock.Unlock()
...
// 由大到小排序，最大元素的排序为 1
func (r *Ranking) Get(key Key) int {
	r.lock.RLock()
...
func (r *Ranking) GetRange(from, to int) ([]Key, []Value) {
	r.lock.RLock()
	defer r.lock.RUnlock()
	if to < from {
		to = from
	}
	return getRange(r.tree, from, to, []Key{}, []Value{})
}

// 获取排名为 n 的 key,score
// n>0 && n<=ranking.len，否则返回 "",0
func (r *Ranking) GetN(n int) (Key, Value) {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
// index 从 1 开始
func (r *Ranking) Walk(handler func(index int, key Key, score Value)) {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
func copyMap(m map[Key]Value) map[Key]Value {
	m2 := map[Key]Value{}
	for k, v := range m {
		m2[k] = v
	}
