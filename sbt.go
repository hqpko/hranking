...
func add(t *tree, key int64, score float64) *tree {
	if t == nil {
...
func del(t *tree, key int64, score float64) *tree {
	if t == nil {
		return nil
	} else if t.key == key {
		if t.left == nil {
...
// 由小到大，从 1 开始，0 表示无排序
func rank(t *tree, key int64, score float64) int {
	if t == nil {
		return 0
	}
	if key == t.key {
...
func walk(t *tree, index int, handler func(index int, key int64, score float64)) int {
	if t == nil {
		return index
	}
...
func getRange(t *tree, from, to int, keys []int64, scores []float64) ([]int64, []float64) {
